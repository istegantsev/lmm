...
file {"logstash_cassandra.conf":
	...
	notify => Exec ['run_logstash_cass'],
     }

exec {"run_logstash_cass":
	path => "/usr/bin/:/bin/:/sbin/:/usr/sbin/",
        command => "/opt/logstash/bin/logstash -f /opt/logstash/conf/logstash_cassandra.conf",
     }
}
...
file {"logstash_magnetodb.conf":
        ...
        notify => Exec ['run_logstash_magneto'],
     }
exec {"run_logstash_magneto":
        path => "/usr/bin/:/bin/:/sbin/:/usr/sbin/",
        command => "/opt/logstash/bin/logstash -f /opt/logstash/conf/logstash_magnetodb.conf",
     }
